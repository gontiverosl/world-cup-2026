...
- **VERIFY's two checks are different in kind:** live vs scratch rebuild must be **exact** (row digests, not just counts — counts miss value drift). The second check is the final-count equality target — see below.
...
**Final counts — fixed equality target, not a floor.** The tournament ended 2026-07-19 at 104/104; counts no longer move. `wc26_verify.py` carries `FINAL_COUNTS = {"player_stats": 3283, "goalkeeper_stats": 215, "matches_played": 104}` (measured 2026-08-03) and `check_counts()` flags any `count != expected` — a move in **either** direction now fails VERIFY, not just a drop. Converted 2026-08-03 from the old mid-tournament `FLOOR` (`2263/149/100`, one-sided `count < floor`), under which a regression that silently dropped ~1,000 `player_stats` rows would have passed. Proven live: a deliberate single-row `player_stats` deletion (3283 → 3282) failed VERIFY with exit code 1 before the row was restored.
