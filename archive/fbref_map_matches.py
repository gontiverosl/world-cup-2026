...
def main():
    ...
    try:
        ...
        for hex_id, date, t1_slug, t2_slug in MATCHES:
            ...
            if not c1 or not c2:
                logging.warning(
                    f"Could not resolve slugs: {t1_slug}={c1}, {t2_slug}={c2} — skipped {hex_id}"
                )
                skipped += 1
                continue
            ...
            if row is None:
                logging.warning(
                    f"No DB match for {hex_id} ({c1} vs {c2} on {date}) — team_home/away may be NULL (R32?)"
                )
                skipped += 1
                continue
            ...
            match_id = row[0]
            cur.execute(
                "UPDATE matches SET fbref_match_id = ? WHERE match_id = ?",
                (hex_id, match_id),
            )
            updated += 1
        ...
        conn.commit()
        logging.info(
            f"fbref_map_matches: {updated} fbref_match_id values set, {skipped} skipped."
        )
